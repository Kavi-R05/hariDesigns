  ...
  return (
    <div className={`reveal ${delay}`} style={{ textAlign: 'center' }}
      ...
      <div style={{ width: 56, height: 56, borderRadius: '50%',
        ...
        <span style={{ fontFamily: "'Playfair Display',serif", fontSize: '.9rem',
                       fontStyle: 'italic', color: hov ? T.white : T.mintDark,
                       transition: 'color .3s' }}>{step.num}</span>
      </div>
      <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: '1.25rem',
                   fontWeight: 400, color: T.charcoal, marginBottom: '.8rem' }}>{step.title}</h3>
      <p style={{ fontSize: '.78rem', lineHeight: 1.9, color: T.warmGray }}>{step.desc}</p>
    </div>
  )
}
    ...
    <section style={{ padding: '8rem 5rem', background: T.white }}>
      <div className="reveal" style={{ textAlign: 'center' }}>
        ...
        <SectionTitle style={{ textAlign: 'center', margin: '1.5rem auto 0' }}>
          Our <em style={{ fontStyle: 'italic', color: T.mintDeep }}>process,</em> refined<br />over 12 years
        </SectionTitle>
      </div>
      <div className="process-grid"
           style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)',
                    gap: '2.5rem', marginTop: '5rem', position: 'relative' }}>
        <div className="process-line"
             style={{ position: 'absolute', top: 28, left: 'calc(12.5% + 16px)',
                      width: 'calc(75% - 32px)', height: 1,
